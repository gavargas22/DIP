...
function enhance_image_with_contrast_stretching(data, slope_value, mid_point_value, display)
    ...
    % Create another Histogram for the new image.
    [counts, binCenters] = imhist(stretched_contrast_image);
    new_image_histogram = figure;
    subplot(1, 1, 1);
    bar(binCenters, counts, 'barWidth', 1);
    grid on;
    %Display the simple statistics
    new_image_mean = ['Mean: ', num2str(im2uint8(mean(mean(stretched_contrast_image))))];
    new_image_std = ['Mean: ', num2str(im2uint8(std(std(stretched_contrast_image))))];
    disp(new_image_mean);
    disp(new_image_std);
...
function perform_histogram_equalization_with_values(image_data, number_of_divisions, display)
    ...
    % Create another Histogram for the new image.
    [counts, binCenters] = imhist(equalized_image);
    new_image_histogram = figure;
    subplot(1, 1, 1);
    bar(binCenters, counts, 'barWidth', 1);
    grid on;
    %Display the simple statistics
    new_image_mean = ['Mean: ', num2str(im2uint8(mean(mean(equalized_image))))];
    new_image_std = ['Mean: ', num2str(im2uint8(std(std(equalized_image))))];
    disp(new_image_mean);
    disp(new_image_std);
